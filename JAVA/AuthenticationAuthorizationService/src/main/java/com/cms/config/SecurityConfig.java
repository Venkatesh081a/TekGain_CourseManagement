...
public class SecurityConfig {  
  ...
  public SecurityFilterChain doFilter(HttpSecurity http) throws Exception {
		...
		.authenticationEntryPoint((request, reponse, authException) -> response.sendError(HttpServletResponse.SC_UNAUTHORIZED, authException.getMessage())))
		.and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS);
    
	http.addFilter(filter,UsernamePasswordAuthenticationFilter.class);
    return http.build();
  }
